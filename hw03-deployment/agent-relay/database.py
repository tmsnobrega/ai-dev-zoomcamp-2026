...
from sqlalchemy import DateTime, ForeignKey, Integer, String, Text, UniqueConstraint, and_, create_engine, event, select
...
def lock_rows(statement: Any, *, skip_locked: bool = False, of: Any = None) -> Any:
    """Add PostgreSQL row locks while keeping the SQLite test path portable."""
    if _is_sqlite(DATABASE_URL):
        return statement
    return statement.with_for_update(skip_locked=skip_locked, of=of)
...
def immediate_transaction() -> Generator[Session, None, None]:
    """Run the transaction boundary used by claim, recovery, and completion.

    SQLite serializes writers with ``BEGIN IMMEDIATE``. PostgreSQL uses its
    normal transaction plus explicit row locks on the affected records.
    """
    ...
    expired_tasks = db.scalars(
        lock_rows(
            ...
            .where(
                ...
                Task.attempts.any(
                    and_(Attempt.outcome == "processing", Attempt.lease_expires_at <= now_db)
                ),
            )
            .order_by(Task.id),
            skip_locked=True,
