"""Persistence operations for Agent Relay with atomic PostgreSQL row locks."""
...
def authenticate(token: str) -> Agent:
    ...
    with immediate_transaction() as db:
        agent = db.scalar(lock_rows(select(Agent).where(Agent.token_hash == token_digest), of=Agent))
...
def create_task(sender_id: str, recipient_id: str, input_text: str, idempotency_key: str | None) -> dict[str, str]:
    ...
    with immediate_transaction() as db:
        recipient = db.scalar(lock_rows(select(Agent).where(Agent.id == recipient_id), of=Agent))
...
def heartbeat(task_id: str, agent_id: str, claim_token: str) -> str:
    with immediate_transaction() as db:
        task = db.scalar(lock_rows(select(Task).where(Task.id == task_id), of=Task))
